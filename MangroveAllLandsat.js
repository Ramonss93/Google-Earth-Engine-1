//--------------------------------------------------------step1-------------------------------------------------------------
// Loading Landsat 5,7,8 collections and computing VIs 
//variable
...
//function innudation
function waterfreq01Msk(image)
{
  return image.select('mNDWI').gte(image.select('EVI'))
    .or(image.select('mNDWI').gte(image.select('NDVI')))
...
var collection3 = ee.ImageCollection('LANDSAT/LT5_SR')
    .filterBounds(studyArea)
    .filterDate(start_date, end_date)
    ...
    .select( 
     ...
     .map(addLandsat7VIs);
print('Landsat5')
print(collection3.size());


//Combine two collection
var  colVIs1 = ee.ImageCollection(collection2.merge(collection1));
var  colVIs = ee.ImageCollection(colVIs1.merge(collection3));
...
//compute landsat water freq
var CntWater = colVIs.map(waterfreq01Msk).sum().rename('CntWater');
Map.addLayer(ee.Image(CntWater),{min:0,max:100},'CntWater',false); 
...
var mvcImage = CntValid.addBands(CntEvgreen).addBands(CntWetland)
                       ...
                       .addBands(CntBarSoRD).addBands(CntWater)
                       .addBands(imgNDVIavg).addBands(imgLSWIavg);
...
var imgFreqWater = mvcImage.select('CntWater').multiply(100).divide(mvcImage.select('CntValid'));//open surface water frequency
var permanentWater = imgFreqWater.gt(75); //permanent water
...
Map.addLayer(permanentWater.updateMask(permanentWater),{"palette":["ff0000"]},'permanentWater',true);
